<aside id="sidebar" class="w-auto overflow-y-auto max-md:absolute max-md:w-auto font-medium max-sm:text-[1rem] md:text-[1.5rem] transition-all duration-300 ease-in-out
  ...
  <nav
      class="flex flex-col justify-between py-[1.25rem] min-h-[calc(100vh-7.5rem)] bg-red-normal shadow-lg mx-auto"
  >
      <ul
      class="flex flex-col flex-1 w-full font-medium leading-none text-orange-light"
      >
      <li>
      <button
        data-id="trang-chu"
      ...
      >
          <div>
              <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-[1.875rem] h-[1.875rem] transition-all hover:stroke-red-normal active:stroke-red-normal ">
              <path d="M15.1499 15.9751C15.0624 15.9626 14.9499 15.9626 14.8499 15.9751C12.6499 15.9001 10.8999 14.1001 10.8999 11.8876C10.8999 9.6251 12.7249 7.7876 14.9999 7.7876C17.2624 7.7876 19.0999 9.6251 19.0999 11.8876C19.0874 14.1001 17.3499 15.9001 15.1499 15.9751Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              <path d="M23.4249 24.2252C21.1999 26.2627 18.2499 27.5002 15 27.5002C11.75 27.5002 8.79995 26.2627 6.57495 24.2252C6.69995 23.0502 7.44995 21.9002 8.78745 21.0002C12.2125 18.7252 17.8124 18.7252 21.2124 21.0002C22.5499 21.9002 23.2999 23.0502 23.4249 24.2252Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              <path d="M15 27.5C21.9036 27.5 27.5 21.9036 27.5 15C27.5 8.09644 21.9036 2.5 15 2.5C8.09644 2.5 2.5 8.09644 2.5 15C2.5 21.9036 8.09644 27.5 15 27.5Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              </svg>
          </div>
      ...
      >
        ...
        <span class="text-[##FFF7F5] max-sm:text-[1rem] md:text-[1.5rem] text-left"
          >Truyện đã lưu</span
        >
      </button>
      </li>
      <li>
      <button
      data-id="quan-ly-truyen"
        onclick="handleSidebarClick(this)"
        class="sidebar-button flex w-full items-center  md:p-5 p-[0.75rem] gap-6 border-l-2 border-solid border-l-red-normal transition-all cursor-pointer
       hover:bg-orange-light hover:text-red-normal"
      >
        <div>
          <svg width="30" height="30" viewBox="0 0 30 31" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-[1.875rem] h-[1.875rem] transition-all hover:stroke-red-normal active:stroke-red-normal ">
            <g clip-path="url(#clip0_797_2468)">
              <path d="M13.7124 3H11.2124C4.9624 3 2.4624 5.5 2.4624 11.75V19.25C2.4624 25.5 4.9624 28 11.2124 28H18.7124C24.9624 28 27.4624 25.5 27.4624 19.25V16.75" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              <path d="M27.35 4.94979C25.8125 8.78729 21.95 14.0123 18.725 16.5998L16.75 18.1748C16.5 18.3623 16.25 18.5123 15.9625 18.6248C15.9625 18.4373 15.95 18.2498 15.925 18.0498C15.8125 17.2123 15.4375 16.4248 14.7625 15.7623C14.075 15.0748 13.25 14.6873 12.4 14.5748C12.2 14.5623 12 14.5498 11.8 14.5623C11.9125 14.2498 12.075 13.9623 12.2875 13.7248L13.8625 11.7498C16.45 8.52479 21.6875 4.63729 25.5125 3.09979C26.1 2.87479 26.675 3.04979 27.0375 3.41229C27.4125 3.78729 27.5875 4.36229 27.35 4.94979Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              <path d="M15.9751 18.6126C15.9751 19.7126 15.5501 20.7626 14.7626 21.5626C14.1501 22.1751 13.3251 22.6001 12.3376 22.7251L9.87512 22.9876C8.53762 23.1376 7.38762 22.0001 7.53762 20.6376L7.80012 18.1751C8.03762 15.9876 9.86262 14.5876 11.8126 14.5501C12.0126 14.5376 12.2126 14.5501 12.4126 14.5626C13.2626 14.6751 14.0876 15.0626 14.7751 15.7501C15.4501 16.4251 15.8251 17.2001 15.9376 18.0376C15.9626 18.2376 15.9751 18.4376 15.9751 18.6126Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
              <path d="M19.7751 15.4748C19.7751 12.8623 17.6626 10.7373 15.0376 10.7373" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
            </g>
            <defs>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleSidebarClick(button) {
    const selectedButtonId = button.getAttribute('data-id');

    // Lưu trạng thái vào LocalStorage
    localStorage.setItem('activeSidebarButton', selectedButtonId);

    // Xóa class active khỏi tất cả các nút khác
    document.querySelectorAll('.sidebar-button').forEach(btn => {
        btn.classList.remove('bg-orange-light', 'text-red-normal');
    });

    // Thêm class active vào nút đã chọn
    button.classList.add('bg-orange-light', 'text-red-normal');

    // Điều hướng tới trang mới
    const targetUrl = '<?= site_url("/") ?>' + selectedButtonId;
    window.location.href = targetUrl;
}

// Áp dụng trạng thái active khi tải lại trang
window.addEventListener('load', () => {
    const activeButtonId = localStorage.getItem('activeSidebarButton');
    const currentPath = window.location.pathname.replace('<?= site_url("/") ?>', '').toLowerCase();
    console.log("Current Path:", currentPath);

    document.querySelectorAll('.sidebar-button').forEach(button => {
        button.classList.remove('bg-orange-light', 'text-red-normal');
        const buttonId = button.getAttribute('data-id').toLowerCase();

        // Nếu buttonID trùng với đường dẫn hoặc với activeButtonId từ LocalStorage
        if (buttonId === currentPath || buttonId === activeButtonId) {
            console.log("Button found:", button);
            button.classList.add('bg-orange-light', 'text-red-normal');
            
            // Cập nhật LocalStorage với đường dẫn thực tế để đồng bộ trạng thái
            localStorage.setItem('activeSidebarButton', buttonId);
        }
    });
});
...
function adjustSidebarTop() {
            ...
            // Áp dụng giá trị top động cho sidebar
